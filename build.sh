...
write_checksum() {
    cd "$OUTPUT_PATH" || exit
    PACKAGE_LIST=$(ls cybertron*.whl) || exit
...
build_cybertron()
{
  ...
  if [[ "X$ENABLE_D" = "Xon" ]]; then
    ...
    export SPONGE_PACKAGE_NAME=cybertron_ascend
    CMAKE_FLAG="-DENABLE_D=ON"
  fi
  if [[ "X$ENABLE_GPU" = "Xon" ]]; then
    echo "build gpu backend"
    export SPONGE_PACKAGE_NAME=cybertron_gpu
    CMAKE_FLAG="-DENABLE_GPU=ON"
    if [[ "X$ENABLE_MD" = "Xon" ]]; then
      CMAKE_FLAG="${CMAKE_FLAG} -DENABLE_MD=ON"
    fi
  fi
  cp -r "${BASEPATH}/cybertron/" "${BASEPATH}/build/cybertron/"
  cp -r "${BASEPATH}/tutorials/" "${BASEPATH}/build/cybertron/"
