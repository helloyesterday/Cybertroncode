...
class SchNetInteraction(Interaction):
    ...
    def __init__(
        ...
        self.atomwise_ac = MLP(dim_filter, dim_feature, [dim_feature,],activation=activation,use_last_activation=False)
    ...
    def print_info(self):
        ...
        if self.use_distances and self.use_bonds:
            ...
            print('---------with edge type: '+('distance' if self.use_distances else 'bonds'))
...
class PhysNetModule(Interaction):
    ...
    def __init__(
        ...
        self.fij_dense = Dense(num_rbf,dim_feature,has_bias=False,activation=None)

        self.gating_vector = Parameter(initializer(Normal(1.0),[dim_feature,]), name="gating_vector")
    ...
    def _interaction_block(self, x, f_ij, c_ij, neighbors, mask):
        ...
        Ggij = self.fij_dense(f_ij*F.expand_dims(c_ij,-1))
...
class NeuralInteractionUnit(Interaction):
    ...
    def __init__(
        ...
        if self.use_dis_filter:
            # self.dis_filter = ResFilter(num_rbf,dim_feature,activation)
        ...
        if self.use_bond_filter:
            self.bond_filter = Dense(dim_feature,dim_feature,has_bias=False,activation=None)

        self.positional_embedding=PositionalEmbedding(dim_feature,self.use_distances,self.use_bonds)
        self.multi_head_attention=MultiheadAttention(dim_feature,n_heads,dim_tensor=4)
    ...
    def print_info(self):
        ...
        if self.use_distances:
            print('---------with filter for distances: '+('yes' if self.use_dis_filter else 'no'))

        if self.use_bonds:
            print('---------with filter for bonds: '+('yes' if self.use_bond_filter else 'no'))
        
        print('---------with multi-haeds: '+str(self.n_heads))
        print('---------with feed forward: '+('yes' if self.use_feed_forward else 'no'))
        if self.max_cycles > 1:
