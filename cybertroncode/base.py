...
class Filter(nn.Cell):
    def __init__(self,
        ...
        if n_hidden > 0:
            ...
            self.dense_layers = MLP(num_rbf,dim_filter,hidden_layers,activation=activation)
        else:
...
class ResFilter(nn.Cell):
    def __init__(self,
        ...
        self.residual = Residual(dim_filter,activation=activation,n_hidden=n_hidden)
...
class SmoothReciprocal(nn.Cell):
    ...
    def construct(self,rij,mask):
        ...
        r_far = F.select( rij > 0, (1.0 - phi2rij) * (1.0 / rij), F.zeros_like(rij) )
...
class PositionalEmbedding(nn.Cell):
    def __init__(self,dim,use_distances=True,use_bonds=False,use_public_layer_norm=True):
        ...
        if not (use_bonds or use_distances):
            raise ValueError('"use_bonds" and "use_distances" cannot be both "False" when initializing "PositionalEmbedding"!')
    ...
    def construct(self,query,key,value,mask=None,cutoff=None):
        ...
        if self.n_heads > 1:
            ...
            if mask is None:
                ...
                # [..., 1, X]
                attention_probs = self.softmax_with_mask(attention_scores,mask)
