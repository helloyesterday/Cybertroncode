...
from cybertroncode.interactions import SchNetInteraction
from cybertroncode.interactions import PhysNetModule
from cybertroncode.interactions import NeuralInteractionUnit
from cybertroncode.base import ResFilter,GraphNorm
...
class DeepGraphMolecularModel(nn.Cell):
    ...
    def __init__(
        ...
        self.atom_embedding = nn.Embedding(num_elements, dim_feature, use_one_hot=True, embedding_table=Normal(1.0))
        ...
        if cutoff_network is not None:
            if cutoff is None:
                ...
                self.cutoff = cutoff
            self.cutoff_network = get_cutoff(cutoff_network,r_max=self.cutoff,return_mask=True,reverse=False)
    ...
    def print_info(self):
        ...
        print('------with cutoff distance: ' + str(self.cutoff) + ' ' + self.unit_length)
        print('------with number of RBF functions: ' + str(self.num_rbf))
        print('------with bond connection: ' + ('Yes' if self.use_bonds else 'No'))
        print('------with feature dimension: ' + str(self.dim_feature))
        print('------with interaction layers:')
        for i,inter in enumerate(self.interactions):
            print('------'+str(i+1)+'. '+inter.name + '(' + self.interaction_typenames[i] + ')')
            inter.print_info()
        print('------with total layers: ' + str(len(self.interactions)))
        print('------output all interaction layers: '+('Yes'if self.read_all_interactions else 'No'))
    ...
    def construct(self, r_ij=1, node_types=None, node_mask=None, neighbors=None, neighbor_mask=None, bonds=None, bond_mask=None):
        ...
        for i in range(n_interactions):
            x = self.interactions[i](x, e, f_ii, f_ij, b_ii, b_ij, c_ij, neighbors, mask)
...
class SchNet(DeepGraphMolecularModel):
    ...
    def __init__(
        ...
        if coupled_interactions:
            ...
            self.interaction_typenames = [ 'D' + str(i) for i in range(self.n_interactions) ]
    ...
    def __init__(
        ...
        if self.interaction_types is not None:
            ...
            for itype in self.interaction_types:
                if itype == 'dis':
                    ...
                    raise ValueError('"interactions" must be "dis", "bond" or "mix"')
        else:
            if self.use_distances and self.use_bonds:
                ...
                raise ValueError('"use_bonds" and "use_distances" cannot be both "False"!')
        
        inter_bond_filter = False
        if self.use_bonds:
            self.bond_embedding = nn.Embedding(num_bond_types, dim_feature, use_one_hot=True, embedding_table=Normal(1.0))
        ...
        if self.use_distances:
            if self.use_public_filter:
                self.dis_filter = ResFilter(num_rbf,dim_feature,self.activation)
        ...
        if coupled_interactions:
            ...
            if self.interaction_types is not None:
                ...
                self.interaction_typenames = [ T + str(i) for i in range(self.n_interactions) ]
