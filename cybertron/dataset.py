...
class DatasetProcessor:
    def __init__(
        self,
        name: str,
        atom_types: np.ndarray,     # (N,A) or (A)
        ...
        if self.atom_types.ndim != 2:
            raise ValueError('The shape of atom_types must be (N_data,N_atoms) or (N_atoms)!')
        ...
        if self.position.ndim != 3:
            raise ValueError('The shape of position must be (N_data,N_atoms,Dim) but got '+str(self.position.shape)+'!')
        ...
        if isinstance(label,(tuple,list)):
            self.label = np.concatenate(tuple(self._get_scalar(l,'label') for l in label),axis=-1)
        else:
        ...
        if force is not None:
            ...
            if self.force.ndim != 3:
                raise ValueError('The shape of force must be (N_data,N_atoms,Dim)!')
            if self.force.shape[0] != self.n_data:
                raise ValueError('The number of force does not match the number of position!')
            if self.force.shape[-1] != self.n_dim:
                raise ValueError('The dimension of force does not match the dimension of position!')
    ...
    def get_normalized_dataset(self,
        ...
        atomwise_scaleshift = self.get_scaleshift_mode(mode)
        if scale is None:
            scale = np.select([atomwise_scaleshift],[self.atom_std],self.mol_std)
        if shift is None:
            shift = np.select([atomwise_scaleshift],[self.atom_avg],self.mol_avg)
        ...
        dataset['scaleshift_mode'] = itemgetter(*atomwise_scaleshift)(self.mode_scaleshift)
    ...
    def get_dataset(self,
        ...
        if num_test < 0 or num_train + num_valid + num_test > self.n_data:
            raise ValueError('The total number of "num_train" ({:d}), "num_valid" ({:d}) and "num_test" ({:d})'
                ' is larger than the number of dataset ({:d}).'.format(num_train,num_valid,num_test,self.n_data))
        ...
        if self.n_label == 1:
            ...
            print('   {:>16.6e}{:>16.6e}{:>12s}'.format(scale[0],shift[0],mode))
        else:
            print('   {:>2s}. {:>16s}{:>16s}{:>12s}'.format('ID','Scale','Shift','Mode'))
            for i in range(self.n_label):
                print('   {:>2d}. {:>16.6e}{:>16.6e}{:>12s}'.format(i,scale[i],shift[i],mode[i]))
        ...
        if norm_train:
            ds_train = self.get_normalized_dataset(atomwise_scaleshift,scale,shift,idx_train,potential_index,dtype)
        else:
            ds_train = self.get_origin_dataset(idx_train,dtype,False)
            ds_train.update(ss_data)

        if norm_valid:
            ds_valid = self.get_normalized_dataset(atomwise_scaleshift,scale,shift,idx_valid,potential_index,dtype)
        else:
            ds_valid = self.get_origin_dataset(idx_valid,dtype,False)
            ds_valid.update(ss_data)

        if norm_test:
            ds_test = self.get_normalized_dataset(atomwise_scaleshift,scale,shift,idx_test,potential_index,dtype)
        else:
    ...
    def save_dataset(self,
        ...
        train_file = prefix + ('_normed' if norm_train else '_origin') + '_trainset_' + str(num_train) + '.npz'
        valid_file = prefix + ('_normed' if norm_valid else '_origin') + '_validset_' + str(num_valid) + '.npz'
    ...
    def label_analysis(self,label,num_atoms=None):
        ...
        def _show_label_analysis(lavg,lstd,lmin,lmax,lmid):
            lrange = lmax - lmin
            if self.n_label == 1:
                print('   {:>16s}{:>16s}{:>16s}{:>16s}{:>16s}{:>16s}'.format('Mean','STD','Min','Max','Median','Range'))
                for i in range(self.n_label):
                    print('   {:>16.6e}{:>16.6e}{:>16.6e}{:>16.6e}{:>16.6e}{:>16.6e}'.format(
                        lavg[0],lstd[0],lmin[0],lmax[0],lmid[0],lrange[0]))
            else:
                print('   ID  {:>16s}{:>16s}{:>16s}{:>16s}{:>16s}{:>16s}'.format('Mean','STD','Min','Max','Median','Range'))
                for i in range(self.n_label):
    ...
    def force_analysis(self,force,atom_mask:np.ndarray=None,tot_atoms:np.ndarray=None):
        # (N,A)
        ...
        print('   {:>16s}{:>16s}{:>16s}{:>16s}'.format('Mean','STD','Max','Median'))
        print('   {:>16.6e}{:>16.6e}{:>16.6e}{:>16.6e}'.format(favg,fstd,fmax,fmid))
        print('-'*105)

        return favg,fstd,fmax,fmid

    def _do_data_analysis(self):
        label = self.label - self.get_label_ref(self.atom_types,self.atom_mask)
        mol_info,atom_info = self.label_analysis(label,self.num_atoms)
        (self.mol_avg,self.mol_std,self.mol_min,self.mol_max,self.mol_mid) = mol_info
        (self.atom_avg,self.atom_std,self.atom_min,self.atom_max,self.atom_mid) = atom_info
        if self.force is not None:
    ...
    def _get_scalar(self,data:np.ndarray,name:str):
        scalar = np.array(data,np.float64)
        if scalar.ndim == 1:
            scalar = np.expand_dims(data,0)
        if scalar.ndim != 2:
            raise ValueError('The shape of '+name+' must be (N_data,N_'+name+') or (N_data)')
        if scalar.shape[0] != self.n_data:
            raise ValueError('The number of '+name+' does not match the number of position!')
        return scalar

    def _get_ref(self,data:np.array,n_ref:int,name:str) -> np.ndarray:
