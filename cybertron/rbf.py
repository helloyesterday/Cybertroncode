...
class GaussianBasis(RadicalBasisFunctions):
    ...
    def __init__(
        ...
        if self.delta is None:
            self.offsets = msnp.linspace(self.r_min, self.r_max, self.num_basis, dtype=ms.float32)
...
class LogGaussianBasis(RadicalBasisFunctions):
    def __init__(
        ...
        if isinstance(self.delta,Length):
            raise TypeError('"delta" in Log Gaussian RBF is dimensionless, so its type should not be "Length"')
        ...
        if self.delta is None:
            self.offsets = msnp.linspace(log_rmin,log_rmax,self.num_basis,dtype=ms.float32)
            self.delta = Tensor(log_range/(self.num_basis-1),ms.float32)
        else:
            if self.num_basis is None:
                num_basis = msnp.ceil(log_range/self.delta,ms.int32) + 1
                self.num_basis = get_integer(num_basis)
            self.offsets = msnp.log(self.r_min/self.r_ref) + msnp.arange(0,self.num_basis) * self.delta
    ...
    def set_basis(self, num_basis=None, delta=None):
        ...
        if delta is None:
            log_rmax = msnp.log(self.r_max/self.r_ref,dtype=ms.float32)
            self.num_basis = get_integer(num_basis)
            self.offsets = msnp.linspace(log_rmin,log_rmax,num_basis,dtype=ms.float32)
            ...
            if num_basis is None:
                num_basis = msnp.ceil(log_range/self.delta,ms.int32) + 1
                self.num_basis = get_integer(num_basis)
            self.offsets = msnp.log(self.r_min/self.r_ref) + msnp.arange(0,self.num_basis.asnumpy()) * self.delta
        set_hyper_parameter(self.hyper_param,'delta',self.delta)
        set_hyper_parameter(self.hyper_param,'num_basis',self.num_basis)
        return self
...
def get_rbf(rbf:str=None,r_max=Length(1,'nm'),length_unit='nm') -> RadicalBasisFunctions:
    ...
    if isinstance(rbf, str):
        ...
        if rbf.lower() in _RBF_BY_KEY.keys():
            ...
            raise ValueError("The RBF corresponding to '{}' was not found.".format(rbf))
    else:
